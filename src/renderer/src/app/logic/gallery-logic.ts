...
export function groupAvatars(chars: readonly ApiCharacter[]): AvatarGroup[] {
  ...
  const groups = [...byModel.entries()].map(([model, list]): AvatarGroup => {
    ...
    list.forEach((c) => { if (c.character_name) counts.set(c.character_name, (counts.get(c.character_name) ?? 0) + 1); });
    const name = [...counts.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] ?? titleCase(model);
    ...
    const image = withPicture?.snapshot || (withPicture?.avatar ? `/avatars/${encodeURIComponent(withPicture.avatar)}` : '');
